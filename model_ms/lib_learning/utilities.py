...
class Curried:
    ...
    def __str__(self):
        if len(self.arguments) == 0:
            return "Curried({}/{})".format(self.f, self.arity)
        else:
            return "Curried({}/{}, {})".format(self.f, self.arity, ', '.join(map(str,self.arguments)))
...
def parallelMap(numberOfCPUs, f, *xs, chunksize=None, maxtasksperchild=None, memorySensitive=False,
    ...
    # eprint('actual number of CPUs:', numberOfCPUs)
    ...
    # print('this is PARALLELMAPDATA:', PARALLELMAPDATA)
...
def jsonBinaryInvoke(binary, message):
    ...
    try:
        process = subprocess.Popen(['sh', binary],
...
class PQ:
    ...
    def __iter__(self):
        for _, index in self.h:
            yield self.index2value[index]
...
def parseSExpression(s):
    ...
    def p(n):
        ...
        if s[n] == '(':
            ...
            while True:
                x,n = p(n)
                # if isinstance(x, str) and (x.isdigit() or is_float(x)):
                #     eprint('this is x:', x)
                #     x = float(x)
                # eprint('this is x after:', x)
                l.append(x)
                while n <= len(s) and s[n].isspace(): n += 1
                if n == len(s): raise ParseFailure(s)
                if s[n] == ')':
...
def unparseSExpression(l):
    ...
    for sub in l:
        if isinstance(sub, list):
            output += unparseSExpression(sub)
            output += ' '
        elif isinstance(sub, str):
            output += sub
            if(sub!='#'): # added this line
                output += ' '
