...
```{r}
m.acc = lmer(data = trials, trialScore  ~ repNum + (1 + repNum | gameid) + (1 | towerSet))
summary(m.acc)
...
```
```{r}
m.wc = lmer(data = trials, word_count ~ repNum + (1 + repNum | gameid) + (1 + repNum | towerSet))
summary(m.wc)
```
...
```
```{r}
m.cc = lmer(data = trials, char_count ~ repNum + (1 + repNum | gameid) + (1 + repNum | towerSet))
summary(m.cc)
```
...
```{r}
m.mes = lmer(data = trials, n_messages ~ repNum + (1 + repNum | gameid) + (1 | towerSet))
summary(m.mes)
```
