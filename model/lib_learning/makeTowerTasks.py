...
class SupervisedTower(Task):
    ...
    def logLikelihood(self, e, timeout=None):
        ...
        if yh is not None and yh == self.plan: return 0.
        return NEGATIVEINFINITY

    
def parseTower(s):
    _13 = Program.parse("1x2")
    _31 = Program.parse("2x1")
...
def mytask():
    C = SupervisedTower("C", "(h (l 1) v v (r 1) h)")
    ...
    return [C, L, Pi, CL, CPi, LPi, LC, PiC, PiL]
...
if __name__ == "__main__":
    ...
    # SupervisedTower.exportMany("/Users/cogtoolslab/Code/towers/tmp/every_tower.png",ts,shuffle=False)
    
    for j,t in enumerate(ts):
        t.exportImage("/Volumes/Passport/Code/caml/syn/tmp/tower_%d.png"%(j+1),
                      drawHand=False)
        eprint(t.name, t.original)

    # for k,v in dSLDemo().items():
    #     import imageio
    #     imageio.imwrite(f"/Users/cogtoolslab/Code/towers/tmp/tower_dsl_{k}.png", v)

    # exampleTowers = [103,104,105,93,73,
    #                  50,67,35,43,106]
    # SupervisedTower.exportMany("/Users/cogtoolslab/Code/towers/tmp/tower_montage.png",
    ...
    #     SupervisedTower.exportMany("/Users/cogtoolslab/Code/towers/tmp/tower10_%d.png"%n,examples,
