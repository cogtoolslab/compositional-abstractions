...
function setupExperiment() {
    ...
    function getStimListFromMongo(config, callback) { //called in experiments where stimulus subsets are stored in mongo database
        ...
        socket.on('stimulus', data => {
            ...
            setupPhase(trialList, trialList => {
    ...
    setupPhase = function (trialList, callback){
        ...
        phaseTrials = _.map(trialMetadata, trialMetadatum => {
            return metadatumToRefExpTrial(trialMetadatum)
        });

        // randomize order learning trials
        // phaseTrials = _.shuffle(phaseTrials); // DO NOT SHUFFLE FOR THIS EXPERIMENT

        // phaseTrials = psuedoRandomizeTrials(phaseTrials,
    ...
    metadatumToRefExpTrial = function(metadatum) {
        /**
        ...
        trialType = "labelRefExp";

        // select plugin based on trialType
        let trialPlugin = expConfig["trialTypes"][trialType];

        // construct trial object for block display
        let scene = makeScene(metadatum.scene);
